...
const COPY = {
  ...
  de: {
    ...
    progress: (n) =>
      `Sehr gut — du hast bereits ${n}% der Arbeit geschafft. Tolle Leistung.`,
    body:
  ...
  pt: {
    ...
    progress: (n) =>
      `Muito bem — você já concluiu ${n}% do caminho. Excelente trabalho.`,
    body:
  ...
  it: {
    ...
    progress: (n) =>
      `Ottimo — hai già coperto il ${n}% del percorso. Bel lavoro.`,
    body:
  ...
  pl: {
    ...
    progress: (n) =>
      `Brawo — masz już ${n}% pracy za sobą. Doskonała robota.`,
    body:
  ...
  ru: {
    ...
    progress: (n) =>
      `Отлично — вы уже прошли ${n}% пути. Прекрасная работа.`,
    body:
  ...
  // Fallback happens below for any code not listed.
};
...
export default function SummaryBand({
  ...
  // Pick localized copy or fallback to English
